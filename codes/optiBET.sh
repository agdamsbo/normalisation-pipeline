...
mask=MNI152_T1_2mm_brain_mask.nii.gz; #default MNI mask
...
# # this is the first part of â€œstep 4"" in the manuscript
#
...
#
echo "step6 apply inverted nonlinear warp to MNI label: MNI152_T1_2mm_brain_mask for subject ${i}"
applywarp --ref=${i} --in=${FSLDIR}/data/standard/${mask} --warp=${i}_step5.nii.gz --out=${i}_step6 --interp=nn
#
# #### 7. binarize brain extractions ###################################
# ## Binarize the back-projected MNI brain in order to use it to â€œpunch-outâ€ brain extraction (in the next step)
#
