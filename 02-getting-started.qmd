...
4.  In a terminal window, make sure you're in the source directory, where your codes and scans are located. Then run the following:

    ```{bash}
    #| eval: false
    #| echo: true           
    sh 00nemo_prep_pipeline.sh 1mm
    ```

    The script has been tweaked to allow for registrering also to 2mm space and also to only do head/brain, so without a lesion mask. Of you can provide a different lesion mask name pattern, if you so desire: `sh 00nemo_prep_pipeline.sh 1mm 'lesion.nii.gz'`
